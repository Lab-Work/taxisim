...
plot(0,0, type="n", xlim=c(lo_speed_mph,hi_speed_mph+1), ylim=c(0,1), xaxt="n", yaxt="n", , bty="n", cex.main=.8, main="")
        ...
        vals = ((lo_speed_mph*10):(hi_speed_mph*10))/10
	...
	a = c((0:((hi_speed_mph - lo_speed_mph)/5))*50 + 1)
	print(a)
	print(vals[a])
	print(vals[a])
	#axis(1, at=a, labels=round(vals[a], 2))
	#abline(v=a)
	text(x=(hi_speed_mph-lo_speed_mph)/2,y=.8,labels="Velocity (mph)", cex=2, col.main="white")
