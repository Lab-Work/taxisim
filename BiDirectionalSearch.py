...
def bidirectional_dijkstra(start_node, end_node, use_astar=False, use_arcflags=False, max_speed=1.0):
    ...
    while(not forward_pq.empty() and not backward_pq.empty()):
        ...
        #The center node is the node which has the shortest total distance to the origin and destination
        ...
        for link in node.forward_links:
            ...
            #If this is better than the current path, then make the update and add that neighbor to the PQ
    ...
    return (center_node, forward_pq, forward_expanded, backward_pq, backward_expanded)
...
def reconstruct_path(center_node):
    ...
    #2) The links leading from the center_node to the destination, which do not need to be reversed
    
    #Reconstruct first part of the path
    first_part = []
    node = center_node
    while(node.forward_predecessor_link!=None):
    ...
    second_part = []
    node = center_node
    while(node.backward_predecessor_link!=None):
...
def compare_paths(p1, p2):
    ...
    for i in range(max(len(p1), len(p2))):
        if(i < len(p1)):
            p1_str = str(p1[i].origin_node.node_id) + " --> " + str(p1[i].connecting_node.node_id)
            p1_len += p1[i].time
        else:
            p1_str =  "                         "
        
        if(i < len(p2)):
            p2_str = str(p2[i].origin_node.node_id) + " --> " + str(p2[i].connecting_node.node_id)
            p2_len += p2[i].time
        else:
            p2_str =  "                         "
